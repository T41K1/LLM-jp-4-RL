...
def _boxed(ans: str) -> str:
    ...
    return r"assistant final Therefore the final answer is \boxed{" + ans + "}."
...
class VerifyResultShapeTest(unittest.TestCase):
    ...
    def test_uses_last_boxed_answer_only(self):
        response = r"assistant final One failed branch gives \boxed{41}. Therefore the final answer is \boxed{42}."
        res = verify_answer(response, "42")
        self.assertTrue(res.ok)
        self.assertEqual(res.pred, "42")

    def test_ignores_correct_unboxed_intermediate_answer(self):
        response = r"assistant final The value 42 appears in the derivation, but the final answer is \boxed{41}."
        res = verify_answer(response, "42")
        self.assertFalse(res.ok)
        self.assertEqual(res.method, "none")

    def test_no_boxed_is_not_matched(self):
        # final marker が無い出力は Harmony 形式違反として False
        res = verify_answer("I am not sure about this problem.", "42")
        self.assertFalse(res.ok)
        self.assertEqual(res.method, "format_violation")
...
class FulltextFallbackTest(unittest.TestCase):
    ...
    def test_unboxed_final_answer_is_rescued(self):
        ...
        response = "assistant final After simplifying everything, the final answer is 42."
        res = verify_answer(response, "42")
        self.assertTrue(res.ok)
    ...
    def test_fulltext_not_used_when_boxed_present(self):
        ...
        response = r"assistant final The value 42 appears here, but the final answer is \boxed{41}."
        res = verify_answer(response, "42")
        self.assertFalse(res.ok)
        self.assertEqual(res.method, "none")
...
class SolutionClipTest(unittest.TestCase):
    ...
    def test_long_prefix_then_boxed_answer(self):
        response = "assistant final " + "padding " * 5000 + r"Therefore the final answer is \boxed{42}."
        res = verify_answer(response, "42")
        self.assertTrue(res.ok)
        self.assertEqual(res.pred, "42")
